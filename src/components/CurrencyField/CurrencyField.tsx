...
// Figma node: 2644-167114
export interface CurrencyFieldProps extends InputHTMLAttributes<HTMLInputElement> {
  label?: string;
  helperText?: string;
  errorText?: string;
...
const CurrencyField = forwardRef<HTMLInputElement, CurrencyFieldProps>(
  (
    ...
    return (
      <div className={className || 'flex flex-col gap-[var(--8px,8px)] items-start w-full'} data-node-id="2644-167114">
        {label && (
        ...
        <div className={`border-[length:var(--thin,1px)] border-solid content-stretch flex gap-[var(--4px,4px)] items-end min-h-[36px] overflow-clip px-[var(--4px,4px)] py-[var(--2px,2px)] rounded-[var(--8px,8px)] w-full ${containerStyles}`}>
          {currencySymbol && (
            <span className="shrink-0 text-[14px] text-[color:var(--semantics-text-subtle-secondary,#6a6b6d)]">
              {currencySymbol}
            </span>
          )}
          <input
          ...
          />
          {currencyCode && (
            <span className="shrink-0 text-[12px] text-[color:var(--semantics-text-subtle-secondary,#6a6b6d)] px-[var(--4px,4px)]">
              {currencyCode}
            </span>
          )}
        </div>
