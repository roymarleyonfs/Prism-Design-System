...
// Figma node: 2027-21703
export interface MultiSelectProps extends Omit<InputHTMLAttributes<HTMLInputElement>, 'type'> {
  label?: string;
  helperText?: string;
  errorText?: string;
    ...
    return (
      ...
      >
        ...
        <div className={`flex flex-col gap-[var(--8px,8px)] items-start ${isVertical ? 'w-full' : 'flex-1 min-w-px'}`}>
          <div
            className={`border-[length:var(--thin,1px)] border-solid content-stretch flex gap-[var(--4px,4px)] items-center min-h-[36px] overflow-clip px-[var(--4px,4px)] py-[var(--2px,2px)] rounded-[var(--8px,8px)] shrink-0 w-full flex-wrap ${containerStyles}`}
          >
            {selectedTags.length > 0 ? (
              selectedTags.map((tag) => (
                <div
                  key={tag.id}
                  className="flex gap-[var(--4px,4px)] items-center px-[var(--8px,8px)] py-[var(--4px,4px)] rounded-[var(--4px,4px)] bg-[var(--semantics-fill-subtle-secondary,#f0f0f0)] text-[13px] font-[var(--Weight-Regular,450)] text-[color:var(--semantics-text-subtle-primary-default,#1b1f22)]"
                >
                  {tag.label}
                  {!isActuallyDisabled && (
                    <button
              ...
              <input
                ref={ref}
                className={`flex-1 bg-transparent border-none outline-none text-[14px] font-[var(--Weight-Regular,450)] leading-[var(--Text-Body-md-LineHeight,18px)] min-w-[120px] placeholder-[color:var(--semantics-text-on-fill-subtle-secondary,#6a6b6d)] ${textColor}`}
