...
// Figma node: 2054-40514
export interface FileAttachmentProps extends InputHTMLAttributes<HTMLInputElement> {
  label?: string;
  helperText?: string;
  errorText?: string;
...
const FileAttachment = forwardRef<HTMLInputElement, FileAttachmentProps>(
  (
    {
      className,
      label,
    ...
    return (
      <div className={className || 'flex flex-col gap-[var(--8px,8px)] items-start w-full'} data-node-id="2054-40514">
        {label && (
        ...
        <div className={`border-[length:var(--thin,1px)] border-solid content-stretch flex gap-[var(--4px,4px)] items-end min-h-[36px] overflow-clip px-[var(--4px,4px)] py-[var(--2px,2px)] rounded-[var(--8px,8px)] w-full ${containerStyles}`}>
          <input
            ...
            className={`flex-1 bg-transparent border-none outline-none text-[14px] font-[var(--Weight-Regular,450)] leading-[var(--Text-Body-md-LineHeight,18px)] ${textColor}`}
          ...
          />
          {fileName && (
            <span className="shrink-0 text-[color:var(--semantics-text-subtle-secondary,#6a6b6d)] text-[12px]">
              ✓
            </span>
          )}
        </div>
