...
// Figma node: 2048-2453
export interface UnitFieldProps extends InputHTMLAttributes<HTMLInputElement> {
  label?: string;
  helperText?: string;
  errorText?: string;
...
const UnitField = forwardRef<HTMLInputElement, UnitFieldProps>(
  (
    {
      className,
      label,
    ...
    return (
      <div className={className || 'flex flex-col gap-[var(--8px,8px)] items-start w-full'} data-node-id="2048-2453">
        {label && (
        ...
        <div className={`border-[length:var(--thin,1px)] border-solid content-stretch flex gap-[var(--4px,4px)] items-end min-h-[36px] overflow-clip px-[var(--4px,4px)] py-[var(--2px,2px)] rounded-[var(--8px,8px)] w-full ${containerStyles}`}>
          ...
          />
          {unit && (
            <span className="shrink-0 text-[14px] text-[color:var(--semantics-text-subtle-secondary,#6a6b6d)] px-[var(--4px,4px)]">
              {unit}
            </span>
          )}
        </div>
