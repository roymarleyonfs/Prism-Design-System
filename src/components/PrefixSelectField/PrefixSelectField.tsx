...
// Figma node: 2874-86217
export interface PrefixSelectFieldProps extends Omit<SelectHTMLAttributes<HTMLSelectElement>, 'multiple'> {
  label?: string;
  helperText?: string;
  errorText?: string;
...
const PrefixSelectField = forwardRef<HTMLSelectElement, PrefixSelectFieldProps>(
  (
    {
      className,
      label,
    ...
    return (
      <div className={className || 'flex flex-col gap-[var(--8px,8px)] items-start w-full'} data-node-id="2874-86217">
        {label && (
        ...
        <div className={`border-[length:var(--thin,1px)] border-solid content-stretch flex gap-[var(--4px,4px)] items-end min-h-[36px] overflow-clip px-[var(--4px,4px)] py-[var(--2px,2px)] rounded-[var(--8px,8px)] w-full ${containerStyles}`}>
          ...
          <div className="content-stretch flex flex-1 gap-[var(--4px,4px)] items-center min-h-[32px] overflow-clip px-[var(--4px,4px)] py-[var(--5px,5px)]">
            <select
              ref={ref}
              className={`flex-1 bg-transparent border-none outline-none text-[14px] font-[var(--Weight-Regular,450)] leading-[var(--Text-Body-md-LineHeight,18px)] appearance-none cursor-pointer ${textColor}`}
...
PrefixSelectField.displayName = 'PrefixSelectField';
